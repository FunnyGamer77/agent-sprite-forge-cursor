...
下面所有命令都假设你已经 `cd` 到 `agent-sprite-forge-cursor/` 目录（也就是这个 README 所在的文件夹）。如果还没拿到代码：
...
- `--force` — 覆盖目标目录里已存在的同名 skill 文件夹。
- `--no-deps` — 跳过 `pip install`（你已经用别的方式管理 Python 依赖时用）。
...
复制到当前工作目录下的 `./.cursor/skills/`，可以跟项目仓库一起被 commit，团队成员 clone 后即可用。

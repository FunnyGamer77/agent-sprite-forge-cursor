...
- Pass the reference image to `GenerateImage` via its `reference_image_paths` argument. If the reference is a local file the agent has not yet read, call `Read` on the image first to load it into context, then pass the same path through `reference_image_paths`.

...
Default to the built-in `GenerateImage` tool for visual assets. Base maps, dressed references, prop sheets, prop sprites, tileset art, parallax layers, and battle backgrounds should come from `GenerateImage` unless the user supplies existing art or explicitly asks for procedural placeholders.

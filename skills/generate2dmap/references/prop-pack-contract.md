...
`scripts/extract_prop_pack.py` performs solid-magenta cleanup on its own (`--threshold` and `--edge-threshold`), so for clean raw sheets you can extract directly without a separate chroma-key pass.

When the raw sheet has antialiased magenta fringe, or when props will be composited over a dark/detailed base, run the bundled soft-matte chroma-key helper first (resolve `<skill_root>` to `~/.cursor/skills/generate2dmap/`):

```bash
python3 <skill_root>/scripts/remove_chroma_key.py \
...
Then run `scripts/extract_prop_pack.py` against the alpha-cleaned sheet:

```bash
python3 <skill_root>/scripts/extract_prop_pack.py \

...
2. Pass the base image to `GenerateImage` via its `reference_image_paths` argument when generating the dressed reference. If the base is an existing local file the agent has not yet read, call `Read` on it first to load it into context, then pass the same path through `reference_image_paths`.
...
```bash
python3 ~/.cursor/skills/generate2dmap/scripts/compose_layered_preview.py \
